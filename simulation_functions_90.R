...
ss_irgt<-function(beta=0.2, eff=0.5){
  #eff=0.5
  vary1=1
  vary0=2
      ...
      for (j in 1:length(rhoy0)){
        for (k in 1:length(m1)){
          for (l in 1:length(m0)){
              ss<-sample_size_irgt(beta=beta, eff=eff[i0], rhoy1=rhoy1[i], rhoy0=rhoy0[j], varx=varx, vary1=vary1, vary0=vary0, m1=m1[k], m0=m0[l], p=0.5)
...
datagen_irgt<-function(beta = 0.20, eff=0.25, rhoy1, rhoy0, m1, m0, n1, n0){
  #Fixed parameters
...
sim_irgt<-function(beta=0.2, eff, rhoy1, rhoy0, varx, vary1, vary0, m1, m0, p=0.5,nsim=1000){
  ...
  ss_irgt<-sample_size_irgt(beta=beta, eff=eff, rhoy1=rhoy1, rhoy0=rhoy0, varx=varx, vary1=vary1, vary0=vary0, m1=m1, m0=m0, p=p)
  ...
  for (s in 1:nsim){
    #Generate data
    df<-datagen_irgt(beta = beta, eff=eff, rhoy1=rhoy1, rhoy0=rhoy0, m1=m1, m0=m0, n1=n1, n0=n0)
    
    if (m0>1){
      #Fit model
      fit0<-try(lme(Y0 ~ W*X, random = list( cluster = pdDiag(~W)),weights = varIdent(form= ~ 1 | W), data=df),silent=T)
      if(class(fit0)=="try-error") next else output0[,,s]<-summary(fit0)$tTable
      
      fit1<-try(lme(Y ~ W*X, random = list( cluster = pdDiag(~W)),weights = varIdent(form= ~ 1 | W), data=df),silent=T)
      if(class(fit1)=="try-error") next else output1[,,s]<-summary(fit1)$tTable
...
ss_crt<-function(beta=0.2, eff=0.5,rhox=0.25){
  #eff=0.5
  vary1=1
  vary0=2
  rhoy1<-c(0.01,0.05,0.10)
  ...
  for (i0 in 1:length(eff)){
    for (i in 1:length(rhoy1)){
      for (j in 1:length(rhoy0)){
        for (k in 1:length(m1)){
            ss<-sample_size_crt(beta = beta, eff=eff[i0], rhoy1=rhoy1[i], rhoy0=rhoy0[j], rhox=rhox, varx=varx, vary1=vary1, vary0=vary0, m1=m1[k], m0=m1[k], p=0.5)
...
datagen_crt<-function(beta = 0.20, eff=0.25, rhoy1, rhoy0, rhox, m1, m0, n1, n0){
  #Fixed parameters
  alpha = 0.05  #type-1 error
                 #type-2 errpr
  #p=p           #rough treatment proportion
...
sim_crt<-function(beta=0.2, eff, rhoy1, rhoy0, rhox, varx, vary1, vary0, m1, m0, p=0.5, nsim=1000){
  
  ss_crt<-sample_size_crt(beta = beta, eff=eff, rhoy1=rhoy1, rhoy0=rhoy0, rhox=rhox, varx=varx, vary1=vary1, vary0=vary0, m1=m1, m0=m0, p=p)
  ...
  for (s in 1:nsim){
    #Generate data
    df<-datagen_crt(beta = beta, eff=eff, rhoy1=rhoy1, rhoy0=rhoy0, rhox=rhox, m1=m1, m0=m0, n1=n1, n0=n0)
